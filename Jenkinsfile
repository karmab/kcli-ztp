...
", description: ''),
    ...
    string(name: 'virtual_workers_deploy', defaultValue: "False", description: ''),
    string(name: 'cache', defaultValue: "True", description: ''),
    string(name: 'notify', defaultValue: "True", description: ''),
    string(name: 'deploy', defaultValue: "True", description: ''),
...
pipeline {
    ...
    environment {
     ...
     KCLI_PARAMETERS = "-P info=${params.info} -P image=${params.image} -P openshift_image=${params.openshift_image} -P cluster=${params.cluster} -P domain=${params.domain} -P network_type=${params.network_type} -P keys=${params.keys} -P api_ip=${params.api_ip} -P dns_ip=${params.dns_ip} -P ingress_ip=${params.ingress_ip} -P image_url=${params.image_url} -P network=${params.network} -P pool=${params.pool} -P numcpus=${params.numcpus} -P masters=${params.masters} -P workers=${params.workers} -P memory=${params.memory} -P disk_size=${params.disk_size} -P extra_disks=${params.extra_disks} -P rhnregister=${params.rhnregister} -P rhnwait=${params.rhnwait} -P provisioning_interface=${params.provisioning_interface} -P provisioning_net=${params.provisioning_net} -P provisioning_ip=${params.provisioning_ip} -P provisioning_cidr=${params.provisioning_cidr} -P provisioning_range=${params.provisioning_range} -P provisioning_installer_ip=${params.provisioning_installer_ip} -P provisioning_macs=${params.provisioning_macs} -P ipmi_user=${params.ipmi_user} -P ipmi_password=${params.ipmi_password} -P baremetal_net=${params.baremetal_net} -P baremetal_cidr=${params.baremetal_cidr} -P baremetal_macs=${params.baremetal_macs} -P baremetal_ips=${params.baremetal_ips} -P pullsecret=${params.pullsecret} -P notifyscript=${params.notifyscript} -P virtual_masters=${params.virtual_masters} -P virtual_masters_number=${params.virtual_masters_number} -P virtual_masters_numcpus=${params.virtual_masters_numcpus} -P virtual_masters_memory=${params.virtual_masters_memory} -P virtual_masters_mac_prefix=${params.virtual_masters_mac_prefix} -P virtual_workers=${params.virtual_workers} -P virtual_workers_number=${params.virtual_workers_number} -P virtual_workers_numcpus=${params.virtual_workers_numcpus} -P virtual_workers_memory=${params.virtual_workers_memory} -P virtual_workers_mac_prefix=${params.virtual_workers_mac_prefix} -P virtual_workers_deploy=${params.virtual_workers_deploy} -P cache=${params.cache} -P notify=${params.notify} -P deploy=${params.deploy} -P lab=${params.lab} -P disconnected=${params.disconnected} -P registry_user=${params.registry_user} -P registry_password=${params.registry_password} -P nfs=${params.nfs} -P imageregistry=${params.imageregistry} -P build=${params.build} -P go_version=${params.go_version} -P prs=${params.prs} -P imagecontentsources=${params.imagecontentsources} -P fips=${params.fips} -P cas=${params.cas}"
